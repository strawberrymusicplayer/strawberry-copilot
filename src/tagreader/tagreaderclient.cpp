...
void TagReaderClient::ProcessRequest(TagReaderRequestPtr request) {
  ...
  if (TagReaderIsMediaFileRequestPtr is_media_file_request = dynamic_pointer_cast<TagReaderIsMediaFileRequest>(request)) {
    ...
    result = WriteFileBlocking(write_file_request->filename, write_file_request->song, write_file_request->save_tags_options, write_file_request->save_tag_cover_data, write_file_request->id3v2_version);
  }
...
TagReaderResult TagReaderClient::WriteFileBlocking(const QString &filename, const Song &song, const SaveTagsOptions save_tags_options, const SaveTagCoverData &save_tag_cover_data, const ID3v2Version id3v2_version) {

  return tagreader_.WriteFile(filename, song, save_tags_options, save_tag_cover_data, id3v2_version);

}

TagReaderReplyPtr TagReaderClient::WriteFileAsync(const QString &filename, const Song &song, const SaveTagsOptions save_tags_options, const SaveTagCoverData &save_tag_cover_data, const ID3v2Version id3v2_version) {
  ...
  request->id3v2_version = id3v2_version;
